...
class MainWindow(Gtk.Window):
    ...
    def build_prune_and_compress(self, button, gui=True):
        logging.info(f"Computing strategy with MCTS and PRISM")
        self.button_build_isolation.set_sensitive(False)
        start_time_mcts_isolation_build = time.time()

        # Run MCTS
        self.prune_graph(mcts=True, gui=gui)
        self.prune_graph(mcts=False, gui=gui)
    ...
    def prune_graph(self, mcts, gui):
        suffix = f'{self.children_to_keep_entry.get_text()}-successors_' \
                 + f'simulationsize-{self.simulations_per_node_entry.get_text()}'
        # mcts=True will generate a strategy synthesized with Monte Carlo Tree Search
        # mcts=False will generate a PRISM model pruned with MCTS and synthesize a strategy from
        # that PRISM model. Additionally, it will analyze a naive strategy that randomly picks
        # modes until complete isolation.
        if mcts:
            strategy_file = os.path.join(self.output_dir, f"mcts_strategy_{suffix}.prism")
            dot_file = os.path.join(self.output_dir, f"mcts_graph_{suffix}.dot")
            mcts_strat = "--mctsstrat "
            naive = ""
            name = "mcts"
        else:
            strategy_file = os.path.join(self.output_dir, f"prism_strategy_{suffix}.prism")
            dot_file = os.path.join(self.output_dir, f"prism_graph_{suffix}.dot")
            mcts_strat = ""
            naive = "--evaluatenaive "
            name = "mcts_prism"
        report_file = os.path.join(self.output_dir, f"{name}_strategy_report_{suffix}.txt")
        if gui:
            manifold = "| tee"
        else:
            manifold = ">"
        ...
        command = (f'python3 {os.path.join(self.base_directory, "src/mcts.py")} '
                   ...
                   f'{naive}'
                   f'{self.filename}'
                   f'{manifold} {os.path.join(self.output_dir, f"{name}_log_{suffix}.txt")}')
        if gui:
            self.feed_input(f'{command}\n')  # run in gui terminal
        else:
    ...
    def prune_graph_with_initial_state(self, button):
        self.feed_input(f'python3 src/mcts.py '
                        ...
                        f'--outputdir {self.output_dir} '
                        f'--strategyfile {os.path.join(self.output_dir, "isolation_graph_strategy.txt")} '
